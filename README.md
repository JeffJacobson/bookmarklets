...
```javascript
javascript:(()=>{function a(e){let t=typeof e=="string"?new URL(e):e,o=t.pathname.split("/"),s=o[o.length-1];return new URL(`feeds/videos.xml?channel_id=${s}`,t)}function*u(){if(!/youtube\.com/i.test(location.host))throw new Error(`This function is meant for use with YouTube. Current URL, ${location}, does not appear to be on YouTube.`);document.body.querySelector("button#button.style-scope.yt-icon-button[aria-pressed='false']")?.click();let e=document.body.querySelector("ytd-guide-collapsible-entry-renderer.style-scope.ytd-guide-section-renderer");e?.hasAttribute("can-show-more")&&e.querySelector("a")?.click();let t=e?.querySelectorAll("ytd-guide-entry-renderer[line-end-style] a[href]");if(t)for(let o of t)yield{href:new URL(o.href),feedUrl:a(o.href),title:o.title}}function d(e){return[`<outline title="${e.title}">`,`<outline type="rss" title="${e.title}" text="${e.title}" version="RSS"`,`xmlUrl="${e.feedUrl}"`,`htmlUrl="${e.href}"/>`,"</outline>","</outline>"].join("")}function i(){let e=new Array('<opml version="1.0">',"<head>","<title>YouTube feeds</title>",`<dateCreated>${new Date}</dateCreated>`,"</head>","<body>");for(let t of u()){let o=d(t);e.push(o)}return e.push("</body></opml>"),e.join("")}function m(){let e=document.createElement("header"),t=document.createElement("button");return t.innerText="Close",e.append(t),{header:e,closeButton:t}}var l=class extends HTMLDialogElement{constructor(){super(),this.attachShadow({mode:"open"});let{header:t,closeButton:o}=m();o.addEventListener("click",()=>{this.close(),this.remove()},{once:!0,passive:!0}),this.shadowRoot?.append(t)}},r="closeable-dialog";function c(){return customElements.define(r,l,{extends:"dialog"})}try{window.customElements.get(r)||c();let e=i(),t=new Blob([e],{type:"text/opml"}),o=URL.createObjectURL(t),s=document.createElement(r),n=document.createElement("a");n.href=o,n.classList.add("opml"),n.text="YouTube subscriptions OPML",s.appendChild(n)}catch{alert("Could not generate OPML.")}})();
```
...
```javascript
javascript:(()=>{var c=document.body.querySelectorAll("a[href^='/watch?']"),n=["list","index"];for(let e of c){let o=new URL(e.href);n.forEach(r=>o.searchParams.delete(r)),e.href=o.href}})();
```
