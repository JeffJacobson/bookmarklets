 ...
 */
function findFeedLinks(): NodeListOf<HTMLLinkElement> {
  const types = [
  ...
  const feedLinks = document.querySelectorAll<HTMLLinkElement>(selector);
  return feedLinks;
}
 ...
 */
function convertLinkToAnchor(linkElement: HTMLLinkElement) {
 ...
 */
function createLinkList(feedLinks: NodeListOf<HTMLLinkElement>) {
 ...
 */
function createListOfFeedAnchors(): HTMLUListElement | null {
  ...
  const feedSelector = feedTypes
    .map((name) => {
      return [`a[title*='${name}'i]`, `a[href*='${name}'i]`].join(",");
    })
    .join(",");
  const feedAnchors =
    document.querySelectorAll<HTMLAnchorElement>(feedSelector);
  ...
  function* convertAnchors(
    anchors: NodeListOf<HTMLAnchorElement> = feedAnchors,
    feedNamesRe = re
  ) {
...
function createDialog() {
  ...
  const existingDialogs = document.body.querySelectorAll(
    `.${bookmarkletClass}`
  );
  if (existingDialogs) {
