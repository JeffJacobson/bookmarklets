...
export function* getSubscriptionList() {
    if (!/youtube\.com/i.test(location.host)) {
        throw new Error(`This function is meant for use with YouTube. Current URL, ${location}, does not appear to be on YouTube.`)
    }
...
export function getPlayedVideosFromList() {
    return [...document.querySelectorAll("#progress.ytd-thumbnail-overlay-resume-playback-renderer[style]")]
        .map(e =>
            e.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement || null
